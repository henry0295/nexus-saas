...
echo "🔄 Deploying fixed welcome.blade.php..."

# Create the welcome.blade.php file with correct Blade syntax
docker compose -f docker-compose.prod.yml exec -T php tee /app/resources/views/welcome.blade.php > /dev/null << 'WELCOME_EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
echo "✓ welcome.blade.php deployed"
...
HTTP_CODE=$(curl -sk https://localhost/ -w "%{http_code}" -o /tmp/response.html)
TITLE=$(grep -o "<title>[^<]*</title>" /tmp/response.html | sed 's/<[^>]*>//g' || echo "Not found")

if [ "$HTTP_CODE" = "200" ]; then
    echo "✅ SUCCESS! Homepage is responding correctly"
    echo "   Status Code: $HTTP_CODE"
    echo "   Page Title: $TITLE"
else
    echo "⚠️  WARNING: Got HTTP $HTTP_CODE"
    echo "   Page Title: $TITLE"
    echo "   Response:"
...
echo ""
echo "Test with: curl -sk https://192.168.101.99/ | head -20"
