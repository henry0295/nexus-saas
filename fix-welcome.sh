#!/bin/bash
# Solución alternativa ultrarobusta para crear welcome.blade.php
# Usa printf en lugar de heredocs para evitar problemas de escaping

cd /opt/nexus-saas

echo "🔄 Eliminando file corrupto..."
docker compose -f docker-compose.prod.yml exec -T php rm -f /app/resources/views/welcome.blade.php

echo "🔄 Navegando al directorio..."
docker compose -f docker-compose.prod.yml exec -T php mkdir -p /app/resources/views

echo "🔄 Creando archivo con printf..."
docker compose -f docker-compose.prod.yml exec -T php sh << 'SHELL_EOF'
cat > /app/resources/views/welcome.blade.php << 'BLADE_EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
docker compose -f docker-compose.prod.yml exec -T php cat /app/resources/views/welcome.blade.php | head -3
...
echo ""
echo "🧪 Prueba de resultado:"
curl -sk https://localhost/ 2>/dev/null | grep -o '<title>[^<]*</title>'
